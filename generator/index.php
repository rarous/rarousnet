...
function scriptHandler(Texy\HandlerInvocation $invocation, $command, array $args, $rawArgs)
{
  ...
  switch ($command) {
    case 'mixcloud-mini':
      ...
      $output = '<iframe loading=lazy width="560" height="60" class="player mixcloud-player mixcloud-player--mini"
        src="https://www.mixcloud.com/widget/iframe/?hide_cover=1&light=1&mini=1&feed=' . $feed . '&' . $params . '"></iframe>';
      return $texy->protect($output, Texy::CONTENT_MARKUP);
    case 'mixcloud':
      $feed = rawurlencode($args[0]);
      $params = htmlspecialchars($args[1] ?? '');
      $output = '<iframe loading=lazy width="560" height="120" class="player mixcloud-player"
    ...
    case 'speakerdeck':
      ...
      $output = '<iframe loading=lazy width="560" height="420" class="player speakerdeck"
        src="https://speakerdeck.com/player/' . $player . '" allowfullscreen data-ratio="1.3333333333333333"></iframe>';
      return $texy->protect($output, Texy::CONTENT_MARKUP);
    case 'spotify':
      ...
      $output = '<iframe loading=lazy class="player spotify-player"
        ...
        width="560" height="152" allowfullscreen
...
$texy->phraseModule->tags['phrase/em'] = 'i'; // add support for syntax for `//Idiomatic text//`
